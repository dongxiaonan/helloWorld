...
public interface AccountMapper {
    ...
    @Select(
        "SELECT account_id, account_name, email_address, password, country, phone_number, enabled " +
        "FROM account " +
    ...
    @Update(
        "UPDATE account " +
        "SET account_name=#{account_name}, email_address=#{emailAddress}, country=#{country}, phone_number=#{phoneNumber}, enabled=#{enabled} " +
        "WHERE account_id=#{account_id}"
    )
    void update(Account account);

    @Select(
        "SELECT account_id, account_name, email_address, password, country, phone_number, enabled FROM account"
    )
    @Results(value = {
